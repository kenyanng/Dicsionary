...
public class Controller extends Dictionary  implements Initializable {
	...
	void wordadd() throws IOException {
		...
		stage.getScene().getStylesheets().add(getClass().getResource(
				"appllication.css").toExternalForm());
		stage.show();
	}
	...
	public void addw(ActionEvent e) {
		...
		if(moi.add(target,explain)) {
			...
			if((target.equals(""))||(explain.equals(""))) {
				JOptionPane.showMessageDialog(null,
						"FAILED ! ! ! \n Target or explain is '' ", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
	...
	void editw(ActionEvent e) {
		...
		if(moi.edit(targetbase,target,explain)) {
			if((target.equals(""))||(explain.equals(""))||targetbase.equals("")) {
				JOptionPane.showMessageDialog(null,
						"FAILED ! ! ! \n Target or explain is '' ", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
			} else {
				JOptionPane.showMessageDialog(null,
	...
	void removew(ActionEvent e) {
		String target = FieldRemoveT.getText().toLowerCase().trim();
		if(moi.remove(target)){
			...
			if(target.equals("")) {
				JOptionPane.showMessageDialog(null,
						"FAILED! ! !\nThat is ''", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
			} else {
				JOptionPane.showMessageDialog(null,
		...
		if (target.equals("")) {
			...
			for (int i = 0; i < currentdic.length; i++) {
				if (dic.get(i).word_target.startsWith(target)) {
					...
					index++;
				} else if (dic.get(i).word_target.startsWith(target)) {
